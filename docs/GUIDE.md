...
- ✅ **메타데이터 포함** - 영상 타입, ID, 길이, 채널명, 고정 댓글, 설명 등 자동 추가
...
├── src/                        # 소스 코드
│   ├── __init__.py            # 패키지 초기화
│   ├── ytdlp_fetcher.py       # yt-dlp 자막/정보 다운로드
│   └── subtitle_processor.py  # VTT 파싱 및 처리
│
├── cli/                    # 실행 스크립트
│   └── main_ytdlp.py          # 메인 CLI 프로그램
│
├── docs/                       # 문서
...
### Q1. 이 스크립트는 어떤 라이브러리를 사용하나요?

**A:** **yt-dlp**를 핵심 라이브러리로 사용합니다. 과거에는 `youtube-transcript-api`도 사용했으나, 안정성과 지원 범위 문제로 현재는 `yt-dlp`만 사용하고 있습니다. `yt-dlp`는 지속적으로 업데이트되며 거의 모든 종류의 유튜브 영상에서 안정적으로 자막과 메타데이터를 추출할 수 있습니다.
